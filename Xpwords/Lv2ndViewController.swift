...
class Lv2ndViewController: UIViewController, Lv2ndDelegate {

    var sceneType:SceneType = SceneType.Num
    var lv2Manager: Lv2ndCircle!
    ...
    override func viewDidLoad() {
        ...
        let (_, level) = HelperAPI.getCurrentScenelv(sceneType, lvName: "LV2")
        (self.view.viewWithTag(114) as! UILabel).text = "完成度：\(level.completed)"

        lv2Manager = Lv2ndCircle(radiusValue: 80.0, inView: self.view)
        lv2Manager.animationType = UIAnimationType.One.toOther(Int(arc4random_uniform(UInt32(6))))
        lv2Manager.sceneType = self.sceneType
        lv2Manager.delegate = self
        lv2Manager.showChioce()
        lv2Manager.show(self.view.viewWithTag(100)!)
    ...
    func loadingText() {
//        let string = seeSentence.sentenceWords as NSString
//        var rangs = [NSRange]()
//        for var i = 0; i < string.length; i++ {
//            let char = string.substringWithRange(NSRange(location: i, length: 1))
//            if char == "?" && i != string.length - 1 {
//                rangs.append(NSRange(location: i, length: 1))
//            }
//        }
//        
//        let mutableAttributedStr = NSMutableAttributedString(string: seeSentence.sentenceWords)
//        for rang in rangs {
...
//        }
//        (self.view.viewWithTag(101) as! UILabel).attributedText = mutableAttributedStr

        (self.view.viewWithTag(100) as! UILabel).text = lv2Manager.sentenceTitle
        (self.view.viewWithTag(102) as! UILabel).text = ""
        
        let words = lv2Manager.sentenceWords
        for var i = 0; i < words.count; i++ {
            let mutableAttributedStr = NSMutableAttributedString(string: words[i])
            mutableAttributedStr.addAttribute(NSForegroundColorAttributeName, value: UIColor.redColor(), range: NSRange(location: 0, length: 1))
            (self.view.viewWithTag(101 + i) as! UILabel).attributedText = mutableAttributedStr
        }
    ...
    func rewardSystem() -> (coin:String, grow:String, star:String) {
        ...
        UserAPI.sharedInstance.updateScene(self.sceneType, lvNum: "LV2", lock: "false", reward: reward)
        ...
        if isMonitor {
            if levelNum == 8 {
                ...
                alertView.goAgain = {
                    ...
                    let (_, level) = HelperAPI.getCurrentScenelv(self.sceneType, lvName: "LV2")
                    (self.view.viewWithTag(114) as! UILabel).text = "完成度：\(level.completed)"
                    
                    self.lv2Manager.removeAll()
            ...
            if self.view.viewWithTag(111)?.hidden == true {
                ...
                alertView.goBack = {
                    self.back()
                    UserAPI.sharedInstance.updateScene(self.sceneType, lvNum: "LV2", lock: "false", reward: (coin: "-10", grow: "-10", star: "0"))
                }
